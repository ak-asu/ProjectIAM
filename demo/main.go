...
func initializationAuthVerifier(configuration *config.Config) (*auth.Verifier, error) {
	...
	logger.Info("Initializing authentication verifier with resolvers:")
	for network, contractAddress := range configuration.SupportedStateContracts {
		...
		didPrefix := chainIDToDIDPrefix(chainID)
		logger.Info(fmt.Sprintf("  Network %s (chain ID %d) -> DID prefix '%s' -> RPC: %s, Contract: %s",
			network, chainID, didPrefix, rpcURL, contractAddress))
		resolvers[didPrefix] = state.NewETHResolver(rpcURL, contractAddress)
	}
	logger.Info(fmt.Sprintf("Total resolvers configured: %d", len(resolvers)))
