...
func (is *IssuerService) IssueCredential(
	...
	logger.Info("=== IssueCredential called ===",
		slog.String("issuerDID", issuerDID.String()),
		slog.String("credentialSubject", fmt.Sprintf("%+v", credentialRequest.CredentialSubject)))

	if credentialRequest.SubjectPosition != "" {
		logger.Error("SubjectPosition validation failed", slog.String("subjectPosition", credentialRequest.SubjectPosition))
		return "", errors.New("subject position must be empty for merklized credentials")
	}
	if credentialRequest.CredentialSchema == "" {
		logger.Error("CredentialSchema validation failed")
	...
	if err != nil {
		logger.Error("Failed to create credential",
			...
			slog.String("issuerDID", issuerDID.String()),
			slog.Any("credentialRequest", credentialRequest))
	...
	logger.Info("Building core claim",
		...
		slog.String("subjectPosition", credentialRequest.SubjectPosition),
		slog.String("merklizedRootPosition", credentialRequest.MerklizedRootPosition))
	...
	if err != nil {
		logger.Error("Failed to build core claim",
			slog.String("error", err.Error()),
			slog.String("issuerDID", issuerDID.String()),
			slog.Any("credentialSubject", verifiableCredential.CredentialSubject))
